...
from .utils import find_images, parse_sample_id, safe_mkdir
...
class LeafColorPipeline:
    ...
    def _init_segmenter(self) -> BaseSegmenter:
        ...
        kwargs = {k: v for k, v in seg.items() if k != "method"}
    ...
    def _aggregate_by_sample(df: pd.DataFrame,
        ...
        # 扁平化列名
        grouped.columns = [f"{col[0]}_{col[1]}" if col[1] != "mean" else col[0]
        ...
        # 添加重复数
        grouped["n_replicates"] = grouped[f"{trait_columns[0]}_count"]
    ...
    def _create_visualization(img_uint8: np.ndarray,
        ...
        for label, val, fmt in key_items:
            ...
            cv2.putText(vis, text, (10, y), font, scale, (255, 255, 255), int(scale * 3))
            cv2.putText(vis, text, (10, y), font, scale, (0, 0, 0), int(scale * 2))
            y += int(25 * scale)
